...
/**
 * Servlet implementation class CommonActionBean4
 */
@WebServlet("/CommonActionBean4")
public class CommonActionBean4 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CommonActionBean4() {
        super();
		...
		// TODO Auto-generated method stub
		 System.out.println("addCar Action bean start");
	        ...
	        if (ServletFileUpload.isMultipartContent(request)) {
		  	    ...
		  	    try {
		  	        ...
		  	        for (FileItem item : items) {
		  	            if (item.isFormField()) {
		  	                ...
		  	                switch (fieldName) {
		  	              case "carId":
   	                        carInputBean.setId(Integer.parseInt(fieldValue));
   	                        break;
		  	        ...
		  	        if (session.getAttribute("username") != null) {
		  	            if (helper.updateRentalCar(carInputBean)) {
		  	    ...
		  	    try {
		  	        request.getRequestDispatcher("AllRentalCarTable.jsp").forward(request, response);
